...
//    [HudView showHudForView:self];
    [self uploadData:0 addStr:addStr selPage:1 postalCode:code];

}
- (void)uploadData:(NSInteger)index addStr:(NSString *)addstr selPage:(NSInteger )selectPage postalCode:(NSString *)code{
    ...
    [NetwortTool getAddAreaListWithParm:@{@"pid":@(index)} Success:^(id  _Nonnull responseObject) {
        ...
        if (selectPage == 1) {
            ...
            for ( int i = 0;i <arr.count;i++ ) {
                ...
                if ([model.areaName isEqual:array[0]]) {
                    ...
                    selIndex = i;
                    [self uploadData:model.areaId addStr:addstr selPage:2 postalCode:code];
        ...
        if (selectPage == 2) {
            ...
            for ( int i = 0;i <arr.count;i++ ) {
                ...
                if ([model.areaName isEqual:array[1]]) {
                    ...
                    [self uploadData:model.areaId addStr:addstr selPage:3 postalCode:code];
                }else{
                    model.selectedState = NO;
                }
        ...
        if (selectPage == 3) {
            ...
            for ( int i = 0;i <arr.count;i++ ) {
                ...
                AddressSelModel *model = [[AddressSelModel alloc] initWithDic:dic];
                if (array.count > 2) {
                    if ([model.areaName isEqual:array[2]] && [model.postalCode isEqual:code]) {
                        selIndex = i;
