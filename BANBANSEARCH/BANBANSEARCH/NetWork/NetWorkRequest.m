...
//                       [self requestFailureWithMsg:dic[@"msg"] code:dic[@"code"]  block:failure];
              ...
              else if ([dic[@"code"] isEqual:@"A00005"]) {
...
+ (void)getWithFormUrl:(NSString *)urlString parameters:(id)parameters success:(void (^)(id _Nonnull))success failure:(void (^)(NSError * _Nonnull))failure{
    ...
    request.allHTTPHeaderFields = @{
        @"Authorization":account.userInfo.accessToken
    };
...
+ (void)postWithFormUrl:(NSString *)urlString
    ...
    [request setValue:account.userInfo.accessToken forHTTPHeaderField:@"Authorization"];
...
+ (NSDictionary *)builderHttpHeader {
    ...
    [sysInfoDic setValue:account.userInfo.accessToken forKey:@"Authorization"];
    return sysInfoDic;
}
...
//    param[@"token"] = account.userInfo.accessToken.length > 0 ? account.userInfo.accessToken : @"-999";
    return param;
}
